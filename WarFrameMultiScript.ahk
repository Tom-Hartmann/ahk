...
for i, key in spamHotkeys { ; Creates hotkeys for each key in the array above
	...
	Hotkey, % "$*~" . key . " Up", % stopSpamBF ; $ to ensure Hotkeys can't trigger themselves
	BoundFuncCache[key] := Func("SendKey").Bind(key)
}
...
SendKey(key){ ; Function-wrapper for the Send Command
	Send, % "{Blind}{" . key . "}"
}
